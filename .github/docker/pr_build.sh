...
#
# This check can be removed when the valgrind 8.0 mem issues 
# are addressed with the current build (9.15 and below).
# Once the PHP8 agent changes are merged in to the branch, it
...
#
  ...
  *8.0*)
    ...
    do_valgrind=
    printf "Skipping valgrind tests on PHP=%s until memory issues are addressed \n" "$PHPS"
    ;;
  *zts*)
